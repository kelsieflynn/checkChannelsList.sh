...
The following makes a range 00.1-59.9. You could customize it further to match exactly, but I'm not doing that here.
```
...
Shell scripts are not a real(tm) as in  (well structured and extensive programming language) like the 3 big P.
perl/python/php
...
Some even say shell scripts are worse than Western wildfires! If so, make sure you got fire suits to wrangle the uncouthed!
